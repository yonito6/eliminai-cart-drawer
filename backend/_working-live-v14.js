...
(function() {
  ...
  var GIFT_HIDE_DISCOUNT_LABEL = CFG.giftHideDiscountLabel !== false;
  ...
  var CCD = {
    ...
    _mergeTiersFromConfig: function(config) {
      ...
      console.log('[CCD] Loaded tiers from backend:', REWARD_TIERS.length, 'tiers, GIFT_HANDLES=', JSON.stringify(GIFT_HANDLES));
    },
      ...
      document.addEventListener('click', function(e) {
        ...
        if (checkoutBtn && !checkoutBtn.classList.contains('ccd-checkout-btn--loading')) {
          ...
          if (hasGift) {
            e.preventDefault();
            e.stopPropagation();
            window.location.href = '/discount/FREECASE?redirect=/checkout';
            return;
          }
        }
      });
        ...
        if (typeof url === 'string' && url.indexOf('/cart/add') !== -1 && opts && opts.method && opts.method.toUpperCase() === 'POST') {
          try {
            ...
            if (body && body.items && Array.isArray(body.items)) {
              ...
              if (!hasProt && !cartAlreadyHasProt && !protectionDone && !_userToggledOff && CFG.protectionDefaultOn !== false && CFG.protectionAutoAdd !== false) {
            ...
            if (!_cartAlreadyHasProt && !protectionDone && !_userToggledOff && CFG.protectionDefaultOn !== false && CFG.protectionAutoAdd !== false) {
              try {
                ...
                if (formId) {
                  ...
                  items.push({ id: PROT_VID, quantity: 1 });
          ...
          var _es = document.querySelector('#CartDrawer .drawer__cart-empty');
            ...
            clone.json().then(function() {
              ...
              (CCD._pendingProtAdd || Promise.resolve()).then(function() {
                ...
                origFetch('/cart.js').then(function(r) { return r.json(); }).then(function(cart) {
                  ...
                  var _shouldAdd = CFG.protectionDefaultOn !== false && CFG.protectionAutoAdd !== false;
    ...
    checkWatchCase: function(cart) {
      ...
      if (GIFT_TIERS.length > 0) {
        ...
        GIFT_TIERS.forEach(function(t) {
          var g = tierGift(t); if (g && g.handle && score >= t.goal) {
        ...
        if (HIGHEST_TIER_ONLY && eligibleGifts.length > 0) {
          ...
          var hg = tierGift(highest); shouldHave[hg.handle] = hg.variantId;
        } else {
          eligibleGifts.forEach(function(t) {
            var g2 = tierGift(t); shouldHave[g2.handle] = g2.variantId;
          });
        }
      ...
      console.log('[CCD GIFT] score=' + score + ' GIFT_TIERS=' + GIFT_TIERS.length + ' shouldHave=' + JSON.stringify(shouldHave) + ' giftInCart=' + JSON.stringify(Object.keys(giftInCart)) + ' HIGHEST_TIER_ONLY=' + HIGHEST_TIER_ONLY);
