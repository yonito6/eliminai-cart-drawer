...
(async () => {
  ...
  const DEMO_THEME = store.demoThemeId;

  const code = fs.readFileSync(path.join(__dirname, '..', 'v14-complete.js'), 'utf8');
  require('./pre-upload-gate')(code, 'upload-demo → DEMO theme');
  console.log('Uploading to DEMO theme', DEMO_THEME, '| Size:', code.length, 'bytes');
