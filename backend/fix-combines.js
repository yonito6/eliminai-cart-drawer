const { PrismaClient } = require('@prisma/client');
const p = new PrismaClient();
async function fix() {
  const store = await p.store.findFirst({
    where: { shopDomain: 'eleganto-3011.myshopify.com' },
    select: { accessToken: true, shopDomain: true }
  });
  const token = store.accessToken;
  const domain = store.shopDomain;

  async function gql(query, variables) {
    const res = await fetch(`https://${domain}/admin/api/2025-01/graphql.json`, {
      method: 'POST',
      headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' },
      body: JSON.stringify({ query, variables })
    });
    return res.json();
  }

  // Enable combining on the Eleganto Case discount
  const result = await gql(`
    mutation discountAutomaticBasicUpdate($id: ID!, $discount: DiscountAutomaticBasicInput!) {
      discountAutomaticBasicUpdate(id: $id, automaticBasicDiscount: $discount) {
        automaticDiscountNode {
          automaticDiscount {
            ... on DiscountAutomaticBasic { title status combinesWith { productDiscounts orderDiscounts shippingDiscounts } }
          }
        }
        userErrors { field message }
      }
    }
  `, {
    id: "gid://shopify/DiscountAutomaticNode/1787496464635",
    discount: {
      combinesWith: {
        productDiscounts: true,
        orderDiscounts: true,
        shippingDiscounts: true
      }
    }
  });
  console.log('Updated Eliminai Gift:', JSON.stringify(result?.data, null, 2));
  const errors = result?.data?.discountAutomaticBasicUpdate?.userErrors;
  if (errors?.length) console.error('Errors:', errors);

  // ALSO enable combining on the BxGy discounts so they allow our discount to stack
  const bxgyIds = [
    'gid://shopify/DiscountAutomaticNode/1588433027323',  // 1+2 FREE
    'gid://shopify/DiscountAutomaticNode/1622934880507',  // 1+1 FREE
    'gid://shopify/DiscountAutomaticNode/1632976961787',  // Watch Oganizer
  ];
  for (const id of bxgyIds) {
    const r = await gql(`
      mutation discountAutomaticBxgyUpdate($id: ID!, $discount: DiscountAutomaticBxgyInput!) {
        discountAutomaticBxgyUpdate(id: $id, automaticBxgyDiscount: $discount) {
          automaticDiscountNode {
            automaticDiscount {
              ... on DiscountAutomaticBxgy { title combinesWith { productDiscounts orderDiscounts shippingDiscounts } }
            }
          }
          userErrors { field message }
        }
      }
    `, {
      id,
      discount: {
        combinesWith: { productDiscounts: true, orderDiscounts: true, shippingDiscounts: true }
      }
    });
    const d = r?.data?.discountAutomaticBxgyUpdate?.automaticDiscountNode?.automaticDiscount;
    console.log(`Updated ${d?.title}: combines=${JSON.stringify(d?.combinesWith)}`);
  }

  console.log('\nAll discounts should now combine. Try the cart again.');
  await p.$disconnect();
}
fix().catch(e => { console.error(e); p.$disconnect(); });
