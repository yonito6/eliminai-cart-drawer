...
(function() {
  ...
  var CCD = {
    ...
    getAdjustedTotal: function(cart) {
      ...
      console.log('[CCD DEBUG] total_price=' + cart.total_price + ' giftCost=' + giftCost + ' adjusted=' + total + ' GIFT_HANDLES=' + JSON.stringify(GIFT_HANDLES));
      if (cart.items) {
        cart.items.forEach(function(item) {
          var isGift = GIFT_HANDLES[item.handle] || item.handle === WATCH_CASE_HANDLE;
          if (isGift) console.log('[CCD DEBUG] GIFT: ' + item.handle + ' price=' + item.price + ' final_line_price=' + item.final_line_price);
        });
      }
