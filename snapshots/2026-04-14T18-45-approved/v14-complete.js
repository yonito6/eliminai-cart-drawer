    ...
    refreshOnOpen: function() {
      ...
      this.loadExperiment(function(config) {
        ...
        .then(function(cart) {
          ...
          if (!protectionDone && !toggling && shouldAutoAdd && rc > 0 && !hasProt) {
            // Nobody added protection yet — add it, then render ONCE with final data
            protectionDone = true;
            CCD.setToggleNoTransition(true);
            fetch('/cart/add.js', {
              method: 'POST',
          ...
          } else if (protectionDone && !hasProt && shouldAutoAdd && rc > 0) {
            // Interceptor already fired the add but it hasn't landed yet.
            // Wait briefly for it to complete, then render ONCE.
            CCD.setToggleNoTransition(true);
            setTimeout(function() {
              fetch('/cart.js')
                .then(function(r) { return r.json(); })
                .then(function(freshCart) {
                  var hasP = freshCart.items.some(function(i) { return i.handle === PROT; });
                  if (!hasP) {
                    // Still missing — add it directly
                    fetch('/cart/add.js', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ items: [{ id: PROT_VID, quantity: 1 }] })
                    ...
                    .then(function() { return fetch('/cart.js'); })
                    .then(function(r2) { return r2.json(); })
                    .then(function(finalCart) { CCD.refresh(finalCart); })
                    .catch(function() { CCD.refresh(freshCart); });
                  } else {
                    CCD.refresh(freshCart);
                  }
                })
