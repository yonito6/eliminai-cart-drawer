...
(function() {
  ...
  var CCD = {
    ...
    renderDrawerShell: function() {
      // Hide any theme cart drawer elements — inject CSS so theme JS can't override
      var hideStyle = document.createElement('style');
      hideStyle.textContent = '#CartDrawer-theme-original, #CartDrawer-theme-original * { display: none !important; visibility: hidden !important; pointer-events: none !important; height: 0 !important; overflow: hidden !important; }';
    ...
    renderItemHTML: function(item) {
      ...
      if (linePrice === 0 && origLinePrice > 0) {
        // Item made free by discount (BXGY) — show "Free" with strikethrough original price
        ...
        '</div>';
      } else if (origLinePrice > linePrice && linePrice > 0) {
        priceRowHtml = '<div class="ccd-item__price-row">' +
          '<span class="ccd-item__compare-price">' + CCD.fmt(origLinePrice) + '</span>' +
    ...
    renderCartItems: function(cart) {
      ...
      cart.items.forEach(function(item) {
        if (item.handle === PROT) return; // skip protection — shown as toggle in footer
      ...
      .then(function(cart) {
        ...
        if (_isRemoving) {
          ...
          if (_rc === 0) {
            ...
            window.__ccd_block_rebuild = false;
            setTimeout(function() {
              (CCD._origFetch || fetch)("/cart.js").then(function(r) { return r.json(); }).then(function(freshCart) {
    ...
    rebuildDiscountRow: function(cart) {
      var dr = document.querySelector('#CCD-Drawer [data-ccd-discounts]');
      if (!dr) return;
    ...
    checkWatchCase: function(cart) {
      ...
      if (toRemove.length > 0) {
        ...
        .then(function() { return _origF('/cart.js'); })
        .then(function(r) { return r.json(); })
        .then(function(c) { watchCaseBusy = false; CCD.refresh(c); })
    ...
    refreshLight: function(cart) {
      if (cart) window.__ccd_last_cart = cart;
      CCD.updateCartBubble(cart);
      ...
      var pb = document.querySelector('#CCD-Drawer [data-ccd-progress]');
      var ft = document.querySelector('#CCD-Drawer [data-ccd-footer]');
      if (rc === 0) {
    ...
    updateProgress: function(cart) {
      ...
      var msgEl = document.querySelector('#CCD-Drawer [data-ccd-progress-msg]');
      if (msgEl) {
        if (allReached) {
