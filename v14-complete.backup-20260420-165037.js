...
(function() {
  ...
  var CCD = {
    ...
    toggleProtection: function(isChecked) {
      ...
      if (isChecked) {
        // Use /cart/update.js (sets qty=1) NOT /cart/add.js (increments) — prevents qty 2 if protection wasn't fully removed
        var _protVidToAdd = (getProtTier(CCD.getAdjustedTotal(window.__ccd_last_cart || {items:[]})) || {vid:PROT_VID}).vid;
        var _updObj = {}; _updObj[_protVidToAdd] = 1;
        _of('/cart/update.js', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    refreshLight: function(cart) {
      ...
      if (protItem && protItem.quantity > 1) {
        ...
        .then(function(r) { return r.json(); })
        .then(function(fixedCart) { CCD._lastCart = fixedCart; window.__ccd_last_cart = fixedCart; CCD.refreshLight(fixedCart); });
        return; // Don't continue with stale qty>1 data
      }
