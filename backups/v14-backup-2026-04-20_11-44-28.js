...
(function() {
  ...
  (function() {
    ...
    s.textContent = '#CartDrawer, cart-drawer, .cart-drawer, [data-drawer=cart-drawer], .js-cart-drawer, .drawer--cart, .drawer[data-drawer=cart-drawer], .side-cart, #side-cart, .mini-cart-drawer, .ajax-cart__drawer { display: none !important; visibility: hidden !important; pointer-events: none !important; }' +
      ...
      '.ccd-gift-change { display: inline-flex !important; align-items: center !important; gap: 4px !important; font-size: 11px !important; font-weight: 500 !important; color: var(--ccd-gift-change-color, #111) !important; background: none !important; border: 1px solid var(--ccd-gift-change-color, #111) !important; border-radius: 6px !important; padding: 3px 8px !important; cursor: pointer !important; transition: all 0.15s !important; margin-top: 2px !important; opacity: 0.7 !important; }\n' +
      '.ccd-gift-change:hover { opacity: 1 !important; }\n' +
      '.ccd-gift-change svg { width: 12px !important; height: 12px !important; }\n' +
  ...
  var CCD = {
        ...
        '<div class="ccd-fixed-header">' +
          '<div class="ccd-header">' +
            ...
            '<div class="ccd-close" style="display:flex;align-items:center;gap:4px">' +
              (window.Shopify && window.Shopify.shop === "eliminai-test.myshopify.com" ? '<button onclick="window._ccdShowDebug()" style="background:none;border:none;color:#bbb;font:8px monospace;padding:2px 4px;cursor:pointer;opacity:0.3" title="Debug">•</button>' : '') +
    ...
    _reorderProtectionLast: function(cart) {
      // Reorder checkout: Regular items → Gifts → Protection (last)
      if (_protReorderBusy || !cart || !cart.items || cart.items.length < 2) return;
      ...
      for (var i = 0; i < cart.items.length; i++) {
        ...
        if (item.handle === PROT) {
          protection = item;
        } else if (GIFT_HANDLES[item.handle] || item.handle === WATCH_CASE_HANDLE) {
          gifts.push(item);
        } else {
          regular.push(item);
        }
      ...
      // Build desired order
      var desired = regular.concat(gifts);
      if (protection) desired.push(protection);
      
      // Check if already in correct order
      var needsReorder = false;
      for (var j = 0; j < cart.items.length; j++) {
        if (cart.items[j].key !== desired[j].key) { needsReorder = true; break; }
      }
      if (!needsReorder) return;
      
      // Items out of order — remove gifts+protection then re-add in correct order
      var toMove = gifts.concat(protection ? [protection] : []);
      if (toMove.length === 0) return;
      
      console.log('[CCD] Reordering cart for checkout: ' + regular.length + ' regular, ' + gifts.length + ' gifts, ' + (protection ? '1 protection' : 'no protection'));
      _protReorderBusy = true;
      
      // Step 1: Remove all items that need to move (set qty 0)
      var removeOps = toMove.map(function(item) {
        return function() {
      ...
      // Step 2: Re-add in correct order (gifts first, then protection)
      var addItems = toMove.map(function(item) {
        return { id: item.variant_id, quantity: item.quantity };
      });
      
      // Execute: remove sequentially, then add all at once
      var chain = Promise.resolve();
      removeOps.forEach(function(op) { chain = chain.then(op); });
      ...
      chain
        ...
        .then(function(newCart) {
          _protReorderBusy = false;
          CCD._lastCart = newCart;
          window.__ccd_last_cart = newCart;
          console.log('[CCD] Reorder complete. New order: ' + newCart.items.map(function(i){return i.handle}).join(', '));
        })
        .catch(function(e) {
          _protReorderBusy = false;
          console.error('[CCD] Reorder failed:', e);
        });
    },
      ...
      giftItems.forEach(function(giftCartItem) {
        ...
        if (GIFT_CUSTOMER_CHOICE && GIFT_TIERS.some(function(t) { return tierGifts(t).length > 1; })) {
          if (!giftEl.querySelector('.ccd-gift-change')) {
            ...
            changeBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 4v6h6"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg> ' + GIFT_CHANGE_TEXT;
            changeBtn.addEventListener('click', function(ev) {
              ev.stopPropagation();
              // Remove current gift from cart, reset chosen state, show picker
              var giftKey = giftCartItem.key;
              _giftChosenByCustomer = {};
              _giftPickerShown = false;
              watchCaseBusy = true;
              var _oF = CCD._origFetch || fetch;
              _oF('/cart/change.js', {
    ...
    _doRefresh: function(cart) {
      ...
      // Ensure protection is last item in Shopify cart (for checkout order)
      CCD._reorderProtectionLast(cart);
    ...
    applyScarcity: function(cart) {
      ...
      if (realItems.length > 0) {
        ...
        if (target === 'last') { idx = realItems.length - 1; }
          ...
          // Never jumps. If removed and re-added, badge returns.
          var _lockedVid = CCD._scarcityLockedVid;
          if (_lockedVid) {
