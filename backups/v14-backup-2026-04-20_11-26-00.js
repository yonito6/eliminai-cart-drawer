...
(function() {
  ...
  var CCD = {
    ...
    _reorderProtectionLast: function(cart) {
      ...
      if (!PROT_ENABLED) return;
      var protIdx = -1;
      var lastNonGiftIdx = -1;
      for (var i = 0; i < cart.items.length; i++) {
        if (cart.items[i].handle === PROT) {
          protIdx = i;
        } else {
          lastNonGiftIdx = i;
        }
      }
      // Protection must be the very last item (highest index)
      if (protIdx === -1 || protIdx === cart.items.length - 1) return;
      // Protection is not last — remove and re-add to push it to end
      console.log('[CCD] Reordering protection to last position (was at index ' + protIdx + ' of ' + cart.items.length + ')');
      _protReorderBusy = true;
      var protVid = cart.items[protIdx].variant_id;
      var protKey = cart.items[protIdx].key;
      var _oF = CCD._origFetch || fetch;
      _oF('/cart/change.js', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: protKey, quantity: 0 })
      })
      .then(function() {
        return _oF('/cart/add.js', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ items: [{ id: protVid, quantity: 1 }] })
        });
      })
      ...
      .then(function(newCart) {
        ...
        // Don't call full refresh — just update internal state
        // The visual order in our drawer is already correct (we render protection separately)
      })
      .catch(function() { _protReorderBusy = false; });
    },
